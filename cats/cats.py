...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    returning_word = word_list[0]
    for listed in word_list:
        listed_diff = diff_function(typed_word, listed, limit)
        if listed_diff < min_diff:
            min_diff = listed_diff
            returning_word = listed
...
def report_progress(typed, source, user_id, upload):
    ...
    fenzi = 0
    fenmu = len(source)
    while  fenzi < len(typed) and typed[fenzi] == source[fenzi]:
        fenzi += 1
...
def fastest_words(words_and_times):
    ...
    for wi in word_indices:
        wi_bestp, time_best = 0, get_time(times, 0, wi)
        for pi in player_indices:
            if time_best > get_time(times, pi, wi):
                wi_bestp, time_best = pi, get_time(times, pi, wi)
        word_bestp.append(wi_bestp)

    player_bestw = [[words[wi] for wi in word_indices if word_bestp[wi] == pi] for pi in player_indices]
    return player_bestw
