...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            assert original_ant.can_contain(self) or self.can_contain(original_ant), 'Too many ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while current_place is not None:
            if current_place.bees and current_place.is_hive == False and self.lower_bound <= distance <= self.upper_bound:
...
# BEGIN Problem 6
class WallAnt(Ant):
    """WallAnt provides a solid wall that is effective at blocking Bees."""
...
class ScubaThrower(ThrowerAnt):
    """A ThrowerAnt that can survive in water."""
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while current_place is not None:
            ...
            if the_ant is not None:
                the_ant.double()
                if the_ant.is_container and the_ant.ant_contained is not None:
