...
def double_eights(n):
    ...
    def digits_maker(num):
        if num < 10:
            return [num]
        return [num % 10] + digits_maker(num // 10)
    digits_made = digits_maker(n)

    def digit_judger(digits):
        if len(digits) == 1:
            return False
        if digits[0] == digits[1] and digits[0] == 8:
            return True
        return digit_judger(digits[1:])
    return digit_judger(digits_made)
